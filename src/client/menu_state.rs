    ...
    fn key_down_event(
        ...
        if let Some(key_code) = input.keycode {
            match key_code {
                ...
                KeyCode::Return => {
                    match self.selected {
                        i if i < self.options.len() => match &self.options[i][0..] {
                            "Classic Mode" => {
                                return Ok(StateResult::NextState(Box::new(ClassicGame::new(
                                    self.map_size,
                                    self.cell_size,
                                    self.step_time,
                                ))));
                            ...
                            "Pair Mode" => {
                                return Ok(StateResult::NextState(Box::new(PairGame::new(
                                    self.map_size,
                                    self.cell_size,
                                    self.step_time,
                                ))));
                            ...
                            "Online Mode" => {
                                return Ok(StateResult::NextState(Box::new(RoomMenu::new(
                                    ctx,
                                    self.map_size,
                                    self.cell_size,
                                    self.step_time,
