use std::{cell::RefCell, future::Ready, io, mem::swap, net::{SocketAddr, UdpSocket}, rc::Rc, sync::mpsc::{self, channel, Receiver, Sender, TryRecvError}, thread::{self, JoinHandle}, time::Duration};

use bincode::ErrorKind;
use ggez::{graphics::{Canvas, Color, Text}, input::keyboard::{self, KeyCode}, Context, GameError, GameResult};

use crate::{client::MenuState, common::{input_object, Button, GameStateHandler, InputObject, StateResult, StrOption}, net::{message::{self, Message, MessageSocket}, udp_net_thread}};
...
impl RoomState {
	pub fn create_room(server: SocketAddr, socket: UdpSocket, host: SocketAddr, map_size: u32, cell_size: f32, step_time: Duration) -> Result<Self, String> {
        ...
        let set_ready = Rc::new(RefCell::new(StrOption::new("Ready".to_string())));
        {
            ...
            set_ready.borrow_mut().set_key_down_cb(Box::new(move |
			ctx: &mut Context,
        ...
        let set_unready = Rc::new(RefCell::new(StrOption::new("UnReady".to_string())));
        {
            let server = server.clone();
            let sender = out_sender.clone();
            set_unready.borrow_mut().set_key_down_cb(Box::new(move |
                ctx: &mut Context,
                input: keyboard::KeyInput,
                repeated: bool,
                | -> Result<StateResult, ggez::GameError> {
        ...
        let start = Rc::new(RefCell::new(StrOption::new("Start".to_string())));
        {
            let server = server.clone();
            let sender = out_sender.clone();
            start.borrow_mut().set_key_down_cb(Box::new(move |
                ctx: &mut Context,
                input: keyboard::KeyInput,
                repeated: bool,
                | -> Result<StateResult, ggez::GameError> {
        ...
        let exit = Rc::new(RefCell::new(StrOption::new("Exit".to_string())));
        {
            let server = server.clone();
            let sender = out_sender.clone();
            exit.borrow_mut().set_key_down_cb(Box::new(move |
                ctx: &mut Context,
                input: keyboard::KeyInput,
                repeated: bool,
                | -> Result<StateResult, ggez::GameError> {
                    if let Some(key_code) = input.keycode {
                        if key_code == KeyCode::Return {
                            ...
                            sender.send((msg, server));
                            return Ok(StateResult::NextState(Box::new(MenuState::new(map_size, cell_size, step_time))));
                        }
                    }
                    Ok(StateResult::Ok)
                }));
        }
        let buttons: Vec<Rc<RefCell<dyn InputObject>>> = vec![
            start,
            set_ready,
            exit,
        ];
    ...
    pub fn join_room(server: SocketAddr, socket: UdpSocket, players: Vec<(SocketAddr, bool)>, map_size: u32, cell_size: f32, step_time: Duration) -> Result<Self, String> {
		let (net_thread, out_sender, in_receiver) = udp_net_thread::run_with_socket(socket)?;
    ...
    fn on_message(&mut self, msg: Message, addr: SocketAddr) -> Result<crate::common::StateResult, ggez::GameError> {
		...
		match msg {
            // TODO
            Message::StartState => { println!("TODO: start game"); Ok(StateResult::Ok)},
            Message::AddPlayer(player) => { self.players.push((player, false)); Ok(StateResult::Ok) },
    ...
    fn remove_player(&mut self, player: SocketAddr) -> Result<crate::common::StateResult, ggez::GameError> { 
    ...
    fn update_ready(&mut self, addr: SocketAddr, is_ready: bool) -> Result<crate::common::StateResult, ggez::GameError> { 
...
impl GameStateHandler for RoomState {
	...
	fn draw(&mut self, _ctx: &mut Context, canvas: &mut Canvas) -> Result<StateResult, ggez::GameError> {
        ...
        canvas.draw(&room_ip, ggez::graphics::DrawParam::default()
				.dest([100.0, 50.0]));
		// draw buttons
		for (i, button) in self.buttons.iter().enumerate() {
			button.borrow().draw(
				...
				.dest([100.0, 100.0 + i as f32 * 100.0])
			);
		}
		// draw players
		for (i, (addr, is_ready)) in self.players.iter().enumerate() {
			let color = if i == 0 as usize { Color::YELLOW } else { Color::WHITE };
	...
	fn key_down_event(
		...
		match self.buttons[self.selected].borrow_mut().key_down_event(ctx, input, repeated)? {
