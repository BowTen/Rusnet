...
use crate::net::message::{Message, MessageSocket, StatusCode};
...
pub struct RoomMenu {
	...
	input_boxes: Vec<Rc<RefCell<dyn InputObject>>>,
...
impl RoomMenu {
	pub fn new(ctx: &Context, map_size: u32, cell_size: f32, step_time: Duration) -> Self {
		...
		room_menu.input_boxes[room_menu.selected_box].borrow_mut().focus();
		
		let button = Rc::new(RefCell::new(StrOption::new("OK".to_string())));
		button.borrow_mut().set_scale(35.0);
		{
			let address = room_menu.input_boxes[0].clone();
			let server_password = room_menu.input_boxes[1].clone();
			let room_password = room_menu.input_boxes[2].clone();
		...
		room_menu.input_boxes.push(button);
...
impl GameStateHandler for RoomMenu {
	...
	fn key_down_event(
		...
		if let Some(key_code) = &input.keycode {
			self.input_boxes[self.selected_box].borrow_mut().unfocus();
			...
			self.input_boxes[self.selected_box].borrow_mut().focus();
			self.mode_options[self.selected_mode].focus();

		}
		match self.input_boxes[self.selected_box].borrow_mut().key_down_event(ctx, input, repeated)? {
