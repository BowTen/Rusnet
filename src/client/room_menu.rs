...
impl RoomMenu {
	pub fn new(ctx: &Context, map_size: u32, cell_size: f32, step_time: Duration) -> Self {
		let mut room_menu = RoomMenu{
			...
			input_boxes: [
			vec![
				Rc::new(RefCell::new(InputBox::new(ctx, "Server Address".to_string()))),
				Rc::new(RefCell::new(InputBox::new(ctx, "Server PassWord".to_string()))),
				Rc::new(RefCell::new(InputBox::new(ctx, "Set Room PassWord".to_string()))),
			],
			vec![
				Rc::new(RefCell::new(InputBox::new(ctx, "Server Address".to_string()))),
				Rc::new(RefCell::new(InputBox::new(ctx, "Room PassWord".to_string()))),
			]],
			map_size,
			cell_size,
			step_time,
		...
		room_menu.input_boxes[0][room_menu.selected_box].borrow_mut().focus();
		room_menu.input_boxes[1][room_menu.selected_box].borrow_mut().focus();
		...
		button.borrow_mut().set_scale(35.0);
		{
			...
			button.borrow_mut().set_key_down_cb(Box::new(move |
				ctx: &mut Context,
				...
				if key_code == KeyCode::Return {
					...
					let spbox = server_password.as_any_mut().downcast_mut::<InputBox>().unwrap();
					let rpbox = room_password.as_any_mut().downcast_mut::<InputBox>().unwrap();
					...
					match UdpSocket::bind("127.0.0.1:0") {
						Ok(socket) => {
							let msg = Message::NewRoom { server_password: spswd, room_password: rpswd };
										...
										Message::Response{status, ..} => {
											match status {
												StatusCode::OK => {
													...
													let room = RoomState::create_room(server, socket, host, map_size, cell_size, step_time).unwrap();
													return Ok(StateResult::NextState(Box::new(
														room
													)));
													spbox.set_content("OK".to_string());		
												},
												_ => spbox.set_content("password error".to_string()),
											}
										},
										_ => abox.set_content("unexpected message".to_string()),
									}
								},
								Err(e) =>{
									println!("{:?}", e);
									...
									match rsp {
										Message::Response{status, content} => {
											match status {
												StatusCode::OK => {
													//TODO: return room
													let players = if let ResponseBody::RoomInfo { players } = content {
														players
													}else {
														panic!("err rsponse type");
													};
													let room = RoomState::join_room(server, socket, players, map_size, cell_size, step_time).unwrap();
													return Ok(StateResult::NextState(Box::new(
														room
													)));
													rpbox.set_content("OK".to_string());		
												},
												_ => rpbox.set_content("password error".to_string()),
											}
										},
										_ => abox.set_content("unexpected message".to_string()),
									}
...
impl GameStateHandler for RoomMenu {
	...
	fn key_down_event(
		...
		if let Some(key_code) = &input.keycode {
			self.input_boxes[self.selected_mode][self.selected_box].borrow_mut().unfocus();
			...
			self.input_boxes[self.selected_mode][self.selected_box].borrow_mut().focus();
			self.mode_options[self.selected_mode].focus();

		}
		match self.input_boxes[self.selected_mode][self.selected_box].borrow_mut().key_down_event(ctx, input, repeated)? {
